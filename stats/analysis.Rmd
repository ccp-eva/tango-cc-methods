...
complete_data <- read_csv("../data/data.csv") %>% 
  mutate(age_group = floor(ageinyears))
```
...
training_trials_plot <- all_trials %>% 
  ...
  mutate(
    ...
    trialtype = if_else(community %in% c("Leipzig (Germany)", "Malatya (Türkiye)", "Nyabyeya (Uganda)", "Ocuilan (Mexico)", "Plymouth (UK)", "Pune (India)", "Stanford (USA)", "Windhoek (Namibia)"),
  ...
  facet_wrap(~ community, nrow = 2, scales="free_x") +
  theme(
...
stratified_agecorrected_plot <- strat_age %>% 
  ...
  geom_density_ridges(col = "white", alpha = 1, rel_min_height = 0.01, fill="#29485D") +
  scale_x_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.2)) +
  labs(y = "", x = "Stratified & age-corrected") +
  theme(axis.title.x = element_text(size = 10)) +
  expand_limits(y = c(1, 18.5))
...
ggsave("../figures/tango-cc.png", width = 12, height = 8, scale = 1, bg = "white")
```
