...
```{r sample}
# full sample, check for less than 4 test trials
all_trials %>%
  group_by(subjid) %>%
  summarise(n= n()) %>%
  arrange(n)

# exclude children with less than 4 test trials (1 training 1, 2 training 2, 1 test with audio instructions + 4 test trials = 8 min)
exclude <- all_trials %>%
  group_by(subjid) %>%
  summarise(n = n()) %>%
  filter(n < 8) %>%
...
ggsave("../figures/tango-cc-results.png", width = 12, height = 11, scale = 0.9, bg = "white")
```
